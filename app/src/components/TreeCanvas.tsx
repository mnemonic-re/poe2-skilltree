...
interface Props {
  ...
  onPick: (node: TreeNode | null, isRightClick?: boolean) => void;
}
  ...
  const endPointer = (e: React.PointerEvent) => {
    ...
    if (pointers.current.size === 0) {
      if (dragging.current && !moved.current) {
        ...
        const wx = cam.screenToWorldX(e.clientX);
        const wy = cam.screenToWorldY(e.clientY);
        const node = pickNode(optsRef.current.tree, wx, wy, optsRef.current.selectedAsc, optsRef.current.ascOffset);
        const isRightClick = e.button === 2;
        optsRef.current.onPick(node, isRightClick);
      }
