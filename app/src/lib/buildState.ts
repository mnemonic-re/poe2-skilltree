...
export type BuildAction =
  ...
  | { type: "clickNode"; node: TreeNode; tree: ParsedTree; isRightClick?: boolean }
  | { type: "setMode"; mode: Tag }
  | { type: "setBudget"; n: number }
  | { type: "clear" }
  | { type: "loadBuild"; build: { selectedClass: number | null; selectedAsc: string | null; mode: Tag; alloc: Map<string, Tag>; ascAlloc: Set<string>; baseBudget: number } };
...
export function buildReducer(s: BuildState, a: BuildAction): BuildState {
  switch (a.type) {
    ...
    case "clickNode": {
      const { node, tree, isRightClick } = a;
      const adj = tree.adjacency;
      ...
      if (isRightClick) {
        if (!s.alloc.has(node.key)) return s;
    ...
    case "loadBuild":
      return {
        ...s,
        selectedClass: a.build.selectedClass,
        selectedAsc: a.build.selectedAsc,
        mode: a.build.mode,
        alloc: new Map(a.build.alloc),
        ascAlloc: new Set(a.build.ascAlloc),
        baseBudget: a.build.baseBudget,
      };
    default:
      return s;
  }
