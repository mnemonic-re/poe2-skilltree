...
import { deserializeBuild, type BuildStateInfo } from "./lib/buildSerializer";
...
export default function App() {
  ...
  const pendingBuild = useRef<{ version: Version; build: BuildStateInfo } | null>(null);

  const loadBuild = useCallback((v: Version, buildState: BuildStateInfo) => {
    setVersionState(v);
    dispatch({ type: "loadBuild", build: buildState });
  }, []);

  // Sync hash changes or initial hash
  useEffect(() => {
    const handleHashChange = () => {
      const hash = window.location.hash;
      if (hash.startsWith("#build=") || hash.startsWith("#build-")) {
        const code = hash.replace("#build=", "").replace("#build-", "");
        const deserialized = deserializeBuild(code);
        if (deserialized) {
          if (ready) {
            loadBuild(deserialized.version, deserialized.build);
          } else {
            pendingBuild.current = deserialized;
          }
        }
      }
    };
  ...
  // Load build once trees are ready
  useEffect(() => {
    if (ready && pendingBuild.current) {
      loadBuild(pendingBuild.current.version, pendingBuild.current.build);
  ...
  const handleNodeClick = useCallback(
    (node: TreeNode | null, isRightClick?: boolean) => {
      if (!node) {
        setFocusKey(null);
        return;
      }
      ...
      // Handle clicking a class start node directly to select/switch class
      if (node.classStartIndex && node.classStartIndex.length > 0) {
        const classIdx = node.classStartIndex[0];
        if (selectedClass !== classIdx) {
      ...
      dispatch({ type: "clickNode", node, tree: t, isRightClick });
    },
    [version, selectedClass, selectedAsc, selectAsc, selectClass]
  );
